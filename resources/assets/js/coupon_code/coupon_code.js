...
listenSubmit('#addCouponCodeForm', function (e) {
    ...
    if ($('#percentageType').prop('checked') == true) {
        if ($('#couponDiscount').val() > 100) {
            displayErrorMessage(Lang.get('messages.coupon_code.coupon_code_percent_validation'))
            return false
        }
    }
...
listenClick('.delete-coupon-code', function () {
    ...
    deleteItem(url, Lang.get('messages.coupon_code.coupon_code'))
})
...
listenChange('input[name="type"]', function () {
    ...
    if ($(this).val() == 1) {
        icon.text(Lang.get('messages.coupon_code.flat'))
        editFormIcon.text(Lang.get('messages.coupon_code.flat'))
    } else {
        icon.text('%')
        editFormIcon.text('%')
    }
